...
class target_model(nn.Module):
    ...
    def fit(self, x,annotation, X_raw, size_factor, pretrain_epoch=10,midtrain_epoch=20, K=5, KK=5, alpha=0.1, batch_size=256, lr=0.001, epochs=500,
            ...
            if epoch>=0:
                with torch.no_grad():
                    ...
                    if bimodality:
                        ...
                        current_error = (((y_pred != last_pred)[ensemble_total > ensemble_threshold]).sum()) / len(y_pred_known)
                    else:
        ...
        _, y_pred_final = torch.max(score_bank, 1)
        if bimodality:
           pred_celltype=[]
           for idx, i in enumerate(y_pred_final):
               anno = annotation[i]
